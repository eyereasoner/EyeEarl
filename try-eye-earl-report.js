...
function fetch(testFile, callback) {
  ...
  fs.exists(localFile, function (exists) {
    if (exists)
      ...
      request.get({'uri':testPath + testFile, 'proxy':'http://localhost:8080'},
...
function performTest(test, actionTurtle, callback) {
  var documentURI = testPath + test.action,
      ...
      eye = spawn('eye', ['--swipl', '--nope', '--pass', documentURI,
                          '--wcache', documentURI, testFolder + test.action]),
      output = "",
      error = "";

  eye.stdout.pipe(fs.createWriteStream(resultFile));
  eye.stderr.on('data', function (data) { error += data; });
  eye.stdout.once('end', function () { eye.stdout.finished = true; afterEyeRun(); });
  eye.stderr.once('end', function () { eye.stderr.finished = true; afterEyeRun(); });

  function afterEyeRun() {
    var errorMatch;
...
function compareGraphs(actual, expected, callback) {
  ...
  function (error, results) {
    ...
    if (results.actualContents === results.expectedContents)
      callback(null, true);
    // If not, we check for proper graph equality with EYE
    else
      exec('echo "(<' + expected + '> <' + actual + '>) a <try#Args>." | eye --swipl --nope - --query try.n3',
        function (error, stdout, stderr) {
        callback(error, /"matched"/.test(stdout), stdout);
      });
  });
}
...
var homepage = 'http://eulersharp.sourceforge.net/',
    application = homepage + '#eye',
    profile = 'http://www.agfa.com/w3c/jdroo/card.rdf',
...
function generateEarlReport(tests, callback) {
  ...
  report.once('open', function () {
    ...
    writeln('  doap:download-page <http://sourceforge.net/projects/eulersharp/files/eulersharp/>;');
    ...
    writeln('  dc:title "EYE";');
    writeln('  dc:description "Euler Yet another proof Engine"@en;');
    writeln('  dc:creator <', developer, '>.');
    writeln();

    writeln('<', developer, '> a foaf:Person, earl:Assertor;');
    writeln('  foaf:name "Jos De Roo";');
    writeln('  foaf:homepage <http://www.agfa.com/w3c/jdroo/>;');
    writeln('  foaf:primaryTopicOf <', profile, '>;');
    writeln('  rdfs:isDefinedBy <', profile, '>.');
